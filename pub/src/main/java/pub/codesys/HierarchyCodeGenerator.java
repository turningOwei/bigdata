...
public class HierarchyCodeGenerator {
	 ...
	 * @throws Throwable
	 */
	public synchronized HierarchyCodeNode getHierarchyCode(String currentCode, boolean isSave)
			throws Throwable {
		if (currentCode == null) {
		...
		if (s == 0) {
			str = prefix + String.format("%0" + z + "d", 1)
					+ appendZero(w + z, digit);
			cn.setCode(str);
			cn.setCodeValue(1);
		} else {
			// 找出缺失节点并添加
			for (int i = 1; i <= s; i++) {
				...
				// 如果没有缺失节点则新增
				if (i == s && s == x) {
					str = prefix + String.format("%0" + z + "d", i + 1)
							+ appendZero(w + z, digit);
					cn.setCode(str);
					cn.setCodeValue(i + 1);
				} else if (i < x) { // 有缺失节点则补入
					str = prefix + String.format("%0" + z + "d", i)
							+ appendZero(w + z, digit);
					cn.setCode(str);
					cn.setCodeValue(i);
					break;
